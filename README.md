...
| [视觉偏好](VISUAL_STYLE.md) | 已确认作品的参考方法、按语义选择表达、构图、文字、色彩、素材与阅读节奏 |
...
视觉生成以 `VISUAL_STYLE.md` 为统一入口：`my-design` 是用户明确喜欢的手工作品参考，OpenCode 的指定场景用于参考截图讲解与视觉衔接，不将任一项目整体复制成模板。参考是只读的本地项目，可能未随 Git 提供；具体路径与使用边界见视觉偏好文档。

数据校验与视觉验收分别报告。Agent 能访问预览时先检查实际构图、关键动画状态与衔接，再请用户确认；不能实际预览时明确说明视觉检查未完成。该检查不自动渲染视频或导出 HTML，也不代替用户审批。
