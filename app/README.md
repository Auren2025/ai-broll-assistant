...
[AGENTS.md](../AGENTS.md) 只记录已确认的根本事实和演进原则；本文记录当前实现，视觉制作的当前观察见 [B-roll 视觉文档](../docs/BROLL_VISUAL_STYLE.md)和[演讲演示视觉文档](../docs/PRESENTATION_VISUAL_STYLE.md)。这些实现与方法可以随实际使用继续调整。
...
- 在 Herdr 中启动 `pnpm dev` 等长时间前台服务时，使用当前工作区右上角 Pane 的 Zsh，并保持命令在前台运行，以便直接查看日志、端口和退出状态；不要使用 Fish Pane、`nohup`、`&` 或其他脱离当前会话的方式。
- 如果该 Pane 当前是 Fish，先运行 `zsh`，确认进入 Zsh 后再启动服务。
...
当前默认 B-roll 流程由根目录的 Agent Skills 描述：
...
以下只记录当前实现限制和已知缺口，不表示这些行为必须长期保留。其中多文件保存一致性和写入入口的完整校验仍需要后续改进，不能仅靠文档说明视为已经解决。
